...
export default function AdminMaintenance() {
  ...
  const getStatusStyle = (status) => {
    ...
    return { ...baseStyle, backgroundColor: theme.colors.mutedBg, color: theme.colors.muted }; // Open
  };
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ color: theme.colors.textMuted, fontSize: '0.95rem', marginTop: '8px', marginBottom: 0 }}>
            Track and manage customer maintenance issues efficiently.
          </p>
        </div>
      ...
      <div style={{ 
        ...
        <Table responsive style={{ margin: 0, borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.map(r => (
              ...
              >
                <td style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.colors.border}`, color: theme.colors.text, fontWeight: '500' }}>
                  {r.rental?.user?.name || 'N/A'}
                </td>
                <td style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.colors.border}`, color: theme.colors.text }}>
                  {r.rental?.product?.name || 'N/A'}
                </td>
                <td style={{ padding: '14px 16px', borderBottom: `1px solid ${theme.colors.border}`, color: theme.colors.textMuted, fontSize: '0.9rem', maxWidth: '300px', lineHeight: '1.4' }}>
                  {r.issue}
                </td>
                  ...
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                    {r.status === 'Open' && (
                      <Button 
                        ...
                        onMouseEnter={() => handleBtnHover(`start-${r._id}`, true)}
                        onMouseLeave={() => handleBtnHover(`start-${r._id}`, false)}
                        onClick={() => update(r._id, 'In Progress')}
                        style={{
                          backgroundColor: btnHovers[`start-${r._id}`] ? theme.colors.accentHover : theme.colors.accent,
                          color: '#FFFFFF',
                          border: 'none',
                          borderRadius: theme.radius.sm,
                          padding: '4px 14px',
                          ...
                          boxShadow: 'none',
                          cursor: actionId === r._id ? 'not-allowed' : 'pointer',
                          opacity: actionId === r._id ? 0.6 : 1
                        }}
                      >
                    ...
                    {r.status !== 'Resolved' && (
                      <Button 
                        size="sm"
                        disabled={actionId === r._id}
                        onMouseEnter={() => handleBtnHover(`resolve-${r._id}`, true)}
                        onMouseLeave={() => handleBtnHover(`resolve-${r._id}`, false)}
                        onClick={() => update(r._id, 'Resolved')}
                        style={{
                          backgroundColor: btnHovers[`resolve-${r._id}`] ? theme.colors.success : 'transparent',
                          color: btnHovers[`resolve-${r._id}`] ? '#FFFFFF' : theme.colors.success,
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} style={{ padding: '60px 20px', textAlign: 'center', color: theme.colors.textMuted }}>
                  <div style={{ fontSize: '2.5rem', marginBottom: '12px' }}>🛠️</div>
                  <p style={{ margin: 0, fontSize: '0.95rem' }}>No maintenance requests match this filter</p>
                </td>
              </tr>
            )}
          </tbody>
